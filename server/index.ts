...
function benefitColumnLabelForActivity(column: BenefitColumn, activityTitle: string) {
  ...
  const labels: Record<string, string> = {
    subject: `فهم ${activityTitle}`,
    teaching: `تطبيق ${activityTitle}`,
    confidence: "تعزيز الثقة بالتطبيق",
    teamwork: "تحسين العمل الجماعي",
    classroom: "تنظيم الممارسات",
    technology: "توظيف الأدوات التقنية",
    motivation: "التعزيز والمتابعة"
  };
...
function benefitColumnsFromAi(columns: BenefitColumn[], payload: any, activityTitle: string) {
  ...
  return columns.map((column, index) => {
    ...
    return {
      ...column,
      label: !isAppliedLessonsActivity(activityTitle) && label.includes("الدروس التطبيقية") ? fallback.slice(0, 42) : label
    };
  });
}
