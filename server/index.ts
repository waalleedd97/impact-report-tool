...
function benefitColumnsFromAi(columns: BenefitColumn[], payload: any) {
  ...
  return columns.map((column, index) => {
    ...
    return {
      ...column,
      label: sanitizeBenefitLabel(value, column.label)
    };
  });
}
...
function sanitizeAiReport(payload: any, input: {
  ...
  const generatedBenefitColumns = benefitColumnsFromAi(input.profile.benefitColumns, payload);
  const report = composeReport({
    email: input.email,
    courseTitle: input.courseTitle,
    level: input.level,
  ...
  return {
    ...
    reportTitle: sanitizeSummaryText(
      input.generationOptions?.reportTitle || payload?.reportTitle,
      report.reportTitle || defaultReportTitle(input.courseTitle),
      140
    ),
    summary: {
      ...report.summary,
      impactSummary: sanitizeSummaryText(payload?.impactSummary || aiSummary.impactSummary, report.summary.impactSummary),
      contributionLabel: sanitizeSummaryText(
        aiLabels.contributionLabel || aiSummary.contributionLabel,
        report.summary.contributionLabel || contributionLabelForCourse(input.courseTitle)
      ),
      effectivenessLabel: sanitizeSummaryText(
        aiLabels.effectivenessLabel || aiSummary.effectivenessLabel,
        report.summary.effectivenessLabel || effectivenessLabelForCourse(input.courseTitle)
      ),
      benefitsHeaderLabel: sanitizeSummaryText(
        aiLabels.benefitsHeaderLabel || aiSummary.benefitsHeaderLabel,
        report.summary.benefitsHeaderLabel || benefitsHeaderLabelForCourse(input.courseTitle)
      )
    },
...
اكتب impactSummary بناءً على عنوان النشاط مباشرة، ولا تستخدم عبارة "الدروس التطبيقية" إلا إذا كان هذا هو عنوان النشاط.
اكتب summaryLabels.contributionLabel وsummaryLabels.effectivenessLabel وsummaryLabels.benefitsHeaderLabel بحيث تستبدل اسم النشاط داخل عناوين الخلايا حسب courseTitle.
مثال: إذا كان courseTitle هو "مجتمعات التعلم المهنية" فاكتب "مدى مساهمة مجتمعات التعلم المهنية في تطوير أدائك التدريسي".
اكتب benefitColumnLabels كمفاتيح مطابقة تماماً لمعرفات benefitColumns، واجعل كل تسمية قصيرة ومناسبة لعنوان النشاط. لا تغيّر المعرفات ولا تستخدم تسميات عامة إذا كان عنوان النشاط محدداً.
...
صيغة JSON:
{
  ...
  "summaryLabels": {
    "contributionLabel": "مدى مساهمة عنوان النشاط في تطوير أدائك التدريسي",
    "effectivenessLabel": "فعالية مدى فعالية الأساليب المستخدمة في تنفيذ عنوان النشاط",
